...
public class BookingController {
    ...
    public BookingResponseDto addBooking(@RequestBody @Valid BookingRequestDto bookingDto,
        ...
        log.info("BookingController: addBooking(): start with bookerId={} and bookingDto='{}'", bookerId, bookingDto);
        return bookingService.addBooking(bookingDto, bookerId);
    }

    @PatchMapping("{bookingId}")
    public BookingResponseDto approveBookingByOwnerId(@PathVariable Long bookingId,
                                                      @RequestHeader("X-Sharer-User-Id") Long ownerId,
                                                      @RequestParam Boolean approved) {
        log.info("BookingController: approveBookingByOwnerId(): start with bookingId={} and ownerId={}",
                bookingId, ownerId);
        return bookingService.approveBookingByOwnerId(bookingId, approved, ownerId);
    }
